--LOB(Large OBject)은 TEXT,그래픽,이미지,비디오,사운드 등 구조화되지 않은
대형 데이터를 저장시 사용한다.
--CLOB: 문자 대형 객체 (Character). Oracle Server는 CLOB과 VARCHAR2
사이에 암시적 변환을 수행한다
...
select * from board;
insert into board
(num,userid,subject,passwd,ref,re_step,re_level,content,ip)
values
...
--1번 게시물에 댓글 추가 (만약 100번에 대한 글이 없으면 에러나기 때문에
foreign key설정했었음)
insert into board_comment
(comment_num,board_num,userid,content) values
...
--검색
select * from board where userid like '%kim%';
select * from board where subject like '%kim%';
select * from board where content like '%kim%';
select * from board
where userid like '%kim%' or content like '%park%';


--이름+제목+내용
select * from board
where userid like 'kim%' or subject like 'kim%' or content like 'kim%';


--union(합집합)
select * from board where userid like 'kim%' union
...
--검색된 레코드 카운트
select count(*) from board where userid like '%kim%';
rollback;
commit;