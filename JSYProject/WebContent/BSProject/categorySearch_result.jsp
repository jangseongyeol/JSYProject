...
<meta name="viewport" content="width=device-width, initial-scale=1">
  <link rel="stylesheet" href="../include/css/bootstrap.css">
   ...
   function searchList(page){
	   location.href="${path}/product_servlet/mainPage_search.do?curPage="+page+"&search_option=${search_option}"+"&keyword=${keyword}";
	}
...
<div class="container_search" id="container_search">

</div>

<div id="serchPage">
<div class="row serchPage">
<h2 class="text-center"><b style="color:blue;">${keyword}</b> 과 관련된 상품</h2>
<br>
 <c:forEach var="dto" items="${list}">
 <c:choose>
  <c:when test="${dto.productname == dto.productname}">
...
<div class="text-center">
     <ul class="pagination">
      <c:if test="${page.curBlock > 1 }">
        <li><a href="#" onclick="searchList('1')">[처음]</a></li>
      </c:if>
      <c:if test="${page.curBlock > 1 }">
        <li><a href="#" onclick="searchList('${page.prevPage}')">[이전]</a></li>
      </c:if>
      <c:forEach var="num" begin="${page.blockStart}" end="${page.blockEnd}">
        <c:choose>
          ...
          <c:otherwise>
            <li><a href="#" onclick="searchList('${num}')">${num}</a></li>
          </c:otherwise>
        </c:choose>
      </c:forEach>
      <c:if test="${page.curBlock < page.totBlock}">
        <li><a href="#" onclick="searchList('${page.nextPage}')">[다음]</a></li>
      </c:if>
      <c:if test="${page.curBlock < page.totBlock}">
        <li><a href="#" onclick="searchList('${page.totPage}')">[끝]</a></li>
      </c:if>
  </ul>
 </div>
</div>
