...
<meta name="viewport" content="width=device-width, initial-scale=1">
  ...
  function reviewBoardList() {		
		document.reviewEdit_form.action="${path}/BSProject/reviewBoard_index.jsp";
		document.reviewEdit_form.submit();
	}
  
  function reviewDel() {
		var chk = confirm("정말 삭제하시겠습니까?");
		if (chk) {
			document.reviewEdit_form.action="${path}/reviewboard_servlet/reviewDelete.do";
			document.reviewEdit_form.submit();
		}
	}
  function reviewUpdate() {
		...
		if (chk) {
			document.reviewEdit_form.action="${path}/reviewboard_servlet/reviewUpdate.do";
			document.reviewEdit_form.submit();
		}
	}	
  ...
  function menuList(submenu){
   ...
   .edit_div .text_cotent {
     padding: 0 !important;
   }
   .edit_div .btn-primary {
...
<div class="container edit_div" id="edit_div">
            ...
            <div class="col-xs-12 col-sm-8 col-md-8 col-lg-8 col-md-offset-2 col-sm-offset-2">
                <form method="post" name="reviewEdit_form" enctype="multipart/form-data" class="form-horizontal">
                    ...
                    <div class="form-group">
                      <label for="filename">사진파일</label><br>
                       <c:if test="${dto.filesize > 0 }">
                         <img src="../upload_image/${dto.filename}" class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                             &nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="fileDel">첨부파일 삭제<br> 
                             &nbsp;&nbsp;&nbsp;&nbsp;<b>사진 변경:아래에 파일설정을 다시 해주세요.</b>
                        </c:if>
                        <input type="file" name="file1" style="padding-left: 10px;">
                    </div>
            
              <div class="form-group">
				<label for="content_label">내용</label><br>			
					<textarea class="form-control text_cotent" name="contents" id="contents" rows="10" cols="10">${dto.contents}</textarea>
              </div>
              <div class="form-group text-center">
                        <input type="hidden" name="num" value="${dto.num}">
                        <button onclick="reviewUpdate()" class="btn btn-primary">
                            수정
                        </button>
                        <button onclick="reviewDel()" class="btn btn-warning">
                            삭제
                        </button>
                        <button type="submit" onclick="reviewBoardList()" class="btn btn-dark pull-left">
                            목록
                        </button>
                    </div>
