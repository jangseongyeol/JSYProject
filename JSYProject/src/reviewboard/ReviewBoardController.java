...
public class ReviewBoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		if(url.indexOf("reviewList.do")!=-1){
			...
			String page="/BSProject/reviewBoard_list.jsp";
			...
			String page="/BSProject/reviewBoard_index.jsp";
		...
		}else if(url.indexOf("reviewView.do")!=-1){
			int num=Integer.parseInt(request.getParameter("num"));
...
//			ReviewBoardDTO dto=dao.view(num);
			...
			String page="/BSProject/reviewBoard_view.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//출력페이지 이동
			String page="/BSProject/reviewBoard_comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			request.setAttribute("dto",dto);
			String page="/BSProject/reviewBoard_reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			if(result!=null){//비밀번호가 맞으면
				page="/BSProject/reviewBoard_edit.jsp";
				request.setAttribute("dto", dao.reviewView(num));
				RequestDispatcher rd=request.getRequestDispatcher(page);
				rd.forward(request, response);
			}else {//비밀번호가 틀리면
				page=contextPath+"/reviewboard_servlet/reviewView.do?num="+num+"&message=error";
...
/*                    String page="/reviewboard_servlet/list.do";*/
			String page="/BSProject/reviewBoard_search.jsp";
/*					response.sendRedirect(contextPath+page);*/
				RequestDispatcher rd=request.getRequestDispatcher(page);
				rd.forward(request, response);
		}
