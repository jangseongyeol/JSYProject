 ...
 */
@WebServlet("/managerboard_servlet/*")
public class ManagerBoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		if(url.indexOf("list.do")!=-1){
			...
			String page="/BSProject/manager_board_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
		
		}else if(url.indexOf("insert_board.do")!=-1){
			//파일업로드 처리
			...
			dao.insert(dto);
			String page="/managerboard_servlet/list.do";
...
//			BoardDTO dto=dao.view(num);
			...
			String page="/BSProject/manager_board_edit.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			//출력페이지 이동
			String page="/BSProject/manager_board_comment_list.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			request.setAttribute("dto",dto);
			String page="/BSProject/manager_board_reply.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			if(result!=null){//비밀번호가 맞으면
				page="/BSProject/manager_board_edit.jsp";
				request.setAttribute("dto", dao.view(num));
				RequestDispatcher rd=request.getRequestDispatcher(page);
				rd.forward(request, response);
			}else {//비밀번호가 틀리면
				page=contextPath+"/managerboard_servlet/view.do?num="+num+"&message=error";
			...
			dao.update(dto);
			//페이지 이동
			String page="/BSProject/manager_board_index.jsp";
			response.sendRedirect(contextPath+page);			
			...
			response.sendRedirect(contextPath+page);
				...
				int count=dao.countSearch(search_option,keyword);
				System.out.println(count);
				int curPage=1;
				if(request.getParameter("curPage")!=null) {
					curPage=Integer.parseInt(request.getParameter("curPage"));
				...
				/*					List<BoardDTO> list2=dao.list(start,end);*/	
				System.out.println(list);
				request.setAttribute("list",list);
				request.setAttribute("page", pager);
...
/*                    String page="/board_servlet/list.do";*/
			String page="/BSProject/manager_board_search.jsp";
/*					response.sendRedirect(contextPath+page);*/
				RequestDispatcher rd=request.getRequestDispatcher(page);
				rd.forward(request, response);
		}
